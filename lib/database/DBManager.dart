...
class DBManager {
  ...
  Future _createNotas(Database db) async {
    return await db.execute(Notas.getCreateSQL());
  }

  Future _createFaltas(Database db) async {
    return await db.execute(Faltas.getCreateSQL());
  }

  Future _createMaterias(Database db) async {
    return await db.execute(Materias.getCreateSQL());
  }

  Future _createPeriodos(Database db) async {
  ...
  Future<List<Periodos>> fetchAllPeriodos() async {
    List<Map> result = await _db.query(Periodos.tableName, columns: Periodos.provideColumns);
    List<Periodos> periodos = result.map((it) => Periodos.fromMap(it)).toList();

    for (var p in periodos) {
      p.materias.addAll(await fetchMateriasByPeriodo(p.id));
    }

    return periodos;
  }

  Future<List<Materias>> fetchMateriasByPeriodo(int idPeriodo) async {
    ...
    final Iterable<Materias> materias = result.map((it) => Materias.fromMap(it)).toList();
  ...
  Future<List<Faltas>> fetchFaltasByMateria(int idMateria) async {
    final result = await _db.query(
      Faltas.tableName,
  ...
  Future<List<Notas>> fetchNotasByMateria(int idMaterias) async {
    final result = await _db.query(
      Notas.tableName,
      columns: Notas.provideColumns,
      where: "${Notas.IDMATERIA} = ?",
      whereArgs: [idMaterias],
    );
